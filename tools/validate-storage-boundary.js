...
assert.equal(backup.dataSchemaVersion, 1);
...
assert.equal(parsedBackup.backup.dataSchemaVersion, 1);
assert.equal(context.parseManaSpecBackupText(JSON.stringify({ ...backup, dataSchemaVersion: 2 })).ok, false);
const migrationFixtureDirectory = path.resolve(__dirname, "..", "test-fixtures", "migrations");
const legacyFixture = fs.readFileSync(path.join(migrationFixtureDirectory, "legacy-unversioned-v1.json"), "utf8");
const currentFixture = fs.readFileSync(path.join(migrationFixtureDirectory, "current-v1.json"), "utf8");
const futureFixture = fs.readFileSync(path.join(migrationFixtureDirectory, "future-v2-rejected.json"), "utf8");
assert.equal(context.parseManaSpecBackupText(legacyFixture).ok, true);
assert.equal(context.parseManaSpecBackupText(currentFixture).ok, true);
assert.equal(context.parseManaSpecBackupText(futureFixture).ok, false);
