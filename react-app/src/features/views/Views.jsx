...
import { calculatePortfolioSummary, filterPositionRows, formatMoney, selectPositionRows } from "../../domain/portfolio.js";
...
function SearchPanel({ initialQuery = "", onAdd }) {
  ...
  return <section className="search-panel module-context-card module-context-card--wide module-context-card--search"><form onSubmit={submit}><label><span>Search / Add Candidate</span><input aria-label="Search / Add Candidate" value={query} onChange={event => setQuery(event.target.value)} placeholder="Find card or set #, e.g. MH3 123" /></label></form>{status && <p className="muted">{status}</p>}{results.length > 0 && <div className="search-results">{results.map(card => <button type="button" key={card.id} onClick={() => setChoice(card)}><span>{card.image_uris?.small && <img src={card.image_uris.small} alt="" />}<strong>{card.name}</strong></span><small>{card.set_name} · {card.set.toUpperCase()} #{card.collector_number}<br />{card.prices.usd ? formatMoney(card.prices.usd) : "No nonfoil price"}{card.prices.usd_foil ? ` · Foil ${formatMoney(card.prices.usd_foil)}` : ""}</small></button>)}</div>}<Modal open={Boolean(choice)} title="Add exact printing" onClose={() => setChoice(null)}>{choice && <AddCandidateForm card={choice} onCancel={() => setChoice(null)} onAdd={options => { onAdd(toTrackedCard(choice, options)); setChoice(null); }} />}</Modal></section>;
}
...
export function RadarView() {
  const { state, updateSlice, updateState } = useAppState(); const [params, setParams] = useSearchParams(); const [filters, setFilters] = useState({ ...EMPTY_CARD_FILTERS }); const [pageSize, setPageSize] = useState(25); const [trade, setTrade] = useState(null); const [detail, setDetail] = useState(null); const [detailPanel, setDetailPanel] = useState(""); const notices = useNotices();
  const focusId = params.get("focus") || "";
  const rows = filterTrackedCards(state.radar.filter(item => !focusId || item.id === focusId), filters);
  ...
  const trackedItems = [...state.specs, ...state.radar];
  function latestMarket(item) { return getRelatedRecordsForPrinting(state.marketObservations, item, trackedItems).sort((a, b) => new Date(b.checkedAt) - new Date(a.checkedAt))[0]; }
  function noteCount(item) { return getRelatedRecordsForPrinting(state.cardNotes, item, trackedItems).length; }
  function historyCount(item) { return getRelatedRecordsForPrinting(state.priceSnapshots, item, trackedItems).length; }
  ...
  const columns = [
    { key: "name", label: "Card", minWidth: 150, widthGrow: 1, widthShrink: 1, render: item => <button className="table-link" onClick={() => openDetail(item)}><CardIdentity item={item} /></button> },
    ...
    { key: "rarity", label: "Rarity", align: "center", width: 54, widthShrink: 0, format: item => item.rarity ? item.rarity[0].toUpperCase() : "-" },
...
export function PositionsView() {
  ...
  const summary = calculatePortfolioSummary(positionRows, state.cash); const rows = filterPositionRows(positionRows, { focusId, query: filter });
  const edit = (item, key, value) => updateSlice("specs", list => list.map(row => row.id === item.sourceRecord.id ? { ...row, [key]: number(value) } : row));
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: item => <button className="table-link" disabled={!item.validation.valid} onClick={() => setDetail(item.sourceRecord)}><CardIdentity item={item} />{!item.validation.valid && <ReconciliationValue row={item} />}</button> },
    ...
    { key: "collector_number", label: "#", align: "center", width: 44, widthShrink: 0, format: item => item.collector_number || "-" },
    ...
    { key: "currentPrice", label: "Now", align: "money", width: 60, widthShrink: 0, sortValue: item => item.currentPrice ?? -1, format: item => item.currentPrice === null || item.currentPrice < 0 ? "-" : formatMoney(item.currentPrice) },
    ...
    { key: "holdTime", label: "Hold", align: "center", width: 62, widthShrink: 0, sortValue: item => item.holdTime || "", format: item => item.holdTime || "Set", editor: "input", onEdit: (item, value) => updateSlice("specs", list => list.map(row => row.id === item.sourceRecord.id ? { ...row, holdTime: value } : row)) },
...
export function TransactionsView() {
  const { state } = useAppState(); const [filter, setFilter] = useState(""); const [type, setType] = useState(""); const [pageSize, setPageSize] = useState(25); const [detail, setDetail] = useState(null); const rows = state.transactions.filter(tx => (!type || tx.type === type) && [tx.name, tx.set_code, tx.notes, tx.type].join(" ").toLowerCase().includes(filter.toLowerCase()));
  const buys = state.transactions.filter(tx => tx.type === "BUY"); const sells = state.transactions.filter(tx => tx.type === "SELL");
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: item => <CardIdentity item={item} /> },
    { key: "set_code", label: "Set", align: "center", width: 48, widthShrink: 0, format: item => String(item.set_code || "-").toUpperCase() },
    { key: "collector_number", label: "#", align: "center", width: 48, widthShrink: 0, format: item => item.collector_number || "-" },
  ...
  return <><ViewHeader title="Transactions" description="Compact ledger for buys, sells, balances, fees, and realized outcomes." /><MetricBand items={[{ label: "Buys", value: buys.length, detail: `${formatMoney(bought)} deployed` }, { label: "Sells", value: sells.length, detail: `${formatMoney(sold)} returned` }, { label: "Net Cash Flow", value: signedMoney(sold - bought), detail: "Sells minus buys" }, { label: "Recent Activity", value: state.transactions[0] ? date(state.transactions[0].date) : "-", detail: state.transactions[0]?.name || "No transactions" }]} /><TableFilterPanel title="Filter Transactions" countText={`${rows.length} ${rows.length === 1 ? "transaction" : "transactions"}`} value={filter} onChange={setFilter} placeholder="Card, set, notes" pageSize={pageSize} onPageSizeChange={setPageSize} onReset={() => { setFilter(""); setType(""); }}><label className="filter-control"><span>Type</span><select aria-label="Transaction type" value={type} onChange={event => setType(event.target.value)}><option value="">All types</option><option>BUY</option><option>SELL</option></select></label></TableFilterPanel><TabulatorTable columns={columns} rows={rows} onRowClick={item => { const printingItem = resolveCardDetailPrinting(item, [...state.specs, ...state.radar]); if (printingItem) setDetail({ ...printingItem, currentPrice: printingItem.currentPrice ?? item.price }); }} tableClass="ms-tabulator--transactions" ariaLabel="Transactions ledger" empty="No transactions yet. Buys and sells will appear here automatically." initialSort={[{ column: "name", dir: "asc" }]} pageSize={pageSize} /><CardDetail item={detail} source="positions" onClose={() => setDetail(null)} /></>;
}

export function HistoryView() {
  const { state } = useAppState(); const [filter, setFilter] = useState(""); const [type, setType] = useState(""); const [pageSize, setPageSize] = useState(25); const [detail, setDetail] = useState(null);
  const allEvents = [
  ...
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1 },
    { key: "set_code", label: "Set", align: "center", width: 48, widthShrink: 0, format: item => String(item.set_code || "-").toUpperCase() },
    { key: "collector_number", label: "#", align: "center", width: 48, widthShrink: 0, format: item => item.collector_number || "-" },
    { key: "rarity", label: "Rarity", align: "center", width: 44, widthShrink: 0, format: rarity },
    { key: "color", label: "Color", align: "center", width: 44, widthShrink: 0, format: color },
  ...
  return <><ViewHeader title="History" description="Recent activity and learning trail across transactions, radar, and notes." /><MetricBand items={[{ label: "Events", value: allEvents.length, detail: "Reviewable records" }, { label: "Trades", value: state.transactions.length, detail: "Buy and sell events" }, { label: "Lessons / Review", value: state.cardNotes.length + (state.thesisNotes || []).length, detail: "Saved decision notes" }, { label: "Notes", value: state.cardNotes.length, detail: "Card memory" }]} /><TableFilterPanel title="Filter History" countText={`${events.length} ${events.length === 1 ? "event" : "events"}`} value={filter} onChange={setFilter} placeholder="Event, card, detail" pageSize={pageSize} onPageSizeChange={setPageSize} onReset={() => { setFilter(""); setType(""); }}><label className="filter-control"><span>Type</span><select aria-label="History type" value={type} onChange={event => setType(event.target.value)}><option value="">All events</option><option value="transaction">Transactions</option><option value="radar">Radar</option><option value="note">Notes</option></select></label></TableFilterPanel><TabulatorTable columns={columns} rows={events} onRowClick={event => { const item = resolveCardDetailPrinting(event, [...state.specs, ...state.radar]); if (item) setDetail(item); }} tableClass="ms-tabulator--history" ariaLabel="Activity history" empty="Activity will appear here as you work." initialSort={[{ column: "name", dir: "asc" }]} pageSize={pageSize} /><CardDetail item={detail} source="positions" onClose={() => setDetail(null)} /></>;
}
