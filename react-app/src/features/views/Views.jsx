...
import { getRelatedRecordsForAsset, getRelatedRecordsForPrinting, getRelatedRecordAssetKey, getRelatedRecordPrintingKey, relatedRecordMatchesAsset, resolveAssetDetail, resolveTrackedAsset } from "../../domain/relatedRecords.js";
...
function SealedProductDetail({ item, source, onClose }) {
  ...
  const targetValue = source === "positions" ? owned?.exitTarget : target.entryTarget;
...
export function RadarView() {
  ...
  const columns = [
    ...
    { key: "distance", label: "Δ Target", align: "money", width: 72, widthShrink: 0, sortValue: item => number(item.currentPrice) && number(item.entryTarget) ? (number(item.currentPrice) - number(item.entryTarget)) / number(item.entryTarget) : 999, render: item => { const distance = number(item.currentPrice) && number(item.entryTarget) ? ((number(item.currentPrice) - number(item.entryTarget)) / number(item.entryTarget)) * 100 : null; return <span className={distance === null ? "muted" : distance <= 0 ? "positive" : "negative"}>{distance === null ? "-" : `${distance > 0 ? "+" : ""}${distance.toFixed(1)}%`}</span>; } },
...
export function PositionsView() {
  ...
  const columns = [
    ...
    { key: "distance", label: "Δ", align: "money", width: 52, widthShrink: 0, sortValue: item => number(item.exitTarget) && number(item.currentPrice) ? ((number(item.currentPrice) - number(item.exitTarget)) / number(item.exitTarget)) * 100 : 999, format: item => number(item.exitTarget) && number(item.currentPrice) ? percent(((number(item.currentPrice) - number(item.exitTarget)) / number(item.exitTarget)) * 100) : "-" },
