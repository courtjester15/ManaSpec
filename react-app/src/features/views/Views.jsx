...
export function RadarView() {
  ...
  useEffect(() => {
    ...
    const item = state.radar.find(row => row.id === focusId);
    if (item) openDetail(item, detailRequest === "history" ? "history" : "");
  }, [detailRequest, focusId, state.radar]);
  const compactMoney = value => number(value) ? `$${number(value).toLocaleString(undefined, { maximumFractionDigits: 2 })}` : "-";
  const columns = [
    { key: "name", label: "Card", minWidth: 150, widthGrow: 1, widthShrink: 1, render: item => <button className="table-link" aria-label={`Open details for ${item.name}, ${printing(item)}`} onClick={() => openDetail(item)}><CardIdentity item={item} /></button> },
...
export function PositionsView() {
  ...
  }, [detailRequest, focusId, state.specs]);
  const edit = (item, key, value) => updateSlice("specs", list => list.map(row => row.id === item.sourceRecord.id ? { ...row, [key]: number(value) } : row));
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: item => <button className="table-link" aria-label={`Open details for ${item.name}, ${printing(item)}`} disabled={!item.validation.valid} onClick={() => setDetail(item.sourceRecord)}><CardIdentity item={item} />{!item.validation.valid && <ReconciliationValue row={item} />}</button> },
...
export function SignalsView() {
  ...
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: item => <button className="table-link" aria-label={`Open details for ${item.name}, ${printing(item)}`} onClick={() => setDetail(item)}><CardIdentity item={item} /></button> },
...
export function TransactionsView() {
  const { state } = useAppState(); const [params, setParams] = useSearchParams(); const [filter, setFilter] = useState(""); const [type, setType] = useState(""); const [pageSize, setPageSize] = useState(25); const [detail, setDetail] = useState(null); const focusId = params.get("focus") || "";
  const trackedItems = [...state.specs, ...state.radar];
  const rows = state.transactions.filter(tx => (!focusId || String(tx.id) === focusId) && (!type || tx.type === type) && [tx.name, tx.set_code, tx.notes, tx.type].join(" ").toLowerCase().includes(filter.toLowerCase()));
  ...
  function openTransactionDetail(item) {
    const printingItem = resolveCardDetailPrinting(item, trackedItems);
    if (printingItem) setDetail({ ...printingItem, currentPrice: printingItem.currentPrice ?? item.price });
  }
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: item => resolveCardDetailPrinting(item, trackedItems) ? <button className="table-link" aria-label={`Open details for ${item.name}, ${printing(item)}`} onClick={() => openTransactionDetail(item)}><CardIdentity item={item} /></button> : <CardIdentity item={item} /> },
    { key: "set_code", label: "Set", align: "center", width: 48, widthShrink: 0, format: item => String(item.set_code || "-").toUpperCase() },
    { key: "collector_number", label: "#", align: "center", width: 48, widthShrink: 0, format: item => item.collector_number || "-" },
  ...
  return <><ViewHeader title="Transactions" description="Compact ledger for buys, sells, balances, fees, and realized outcomes." /><MetricBand items={[{ label: "Buys", value: buys.length, detail: `${formatMoney(bought)} deployed` }, { label: "Sells", value: sells.length, detail: `${formatMoney(sold)} returned` }, { label: "Net Cash Flow", value: signedMoney(sold - bought), detail: "Sells minus buys" }, { label: "Recent Activity", value: state.transactions[0] ? date(state.transactions[0].date) : "-", detail: state.transactions[0]?.name || "No transactions" }]} /><TableFilterPanel title="Filter Transactions" countText={`${rows.length} ${rows.length === 1 ? "transaction" : "transactions"}`} value={filter} onChange={value => { if (focusId) setParams({}); setFilter(value); }} placeholder="Card, set, notes" pageSize={pageSize} onPageSizeChange={setPageSize} onReset={() => { setParams({}); setFilter(""); setType(""); }}><label className="filter-control"><span>Type</span><select aria-label="Transaction type" value={type} onChange={event => setType(event.target.value)}><option value="">All types</option><option>BUY</option><option>SELL</option></select></label>{focusId && <button type="button" className="filter-reset-btn" onClick={() => setParams({})}>Show all Transactions</button>}</TableFilterPanel><TabulatorTable columns={columns} rows={rows} onRowClick={openTransactionDetail} tableClass="ms-tabulator--transactions" ariaLabel="Transactions ledger" empty="No transactions yet. Buys and sells will appear here automatically." initialSort={[{ column: "name", dir: "asc" }]} pageSize={pageSize} /><CardDetail item={detail} source="positions" onClose={() => setDetail(null)} /></>;
}
...
export function HistoryView() {
  ...
  const events = allEvents.filter(event => (!focusId || String(event.id) === focusId) && (!type || event.kind === type) && [event.name, event.summary, event.eventType].join(" ").toLowerCase().includes(filter.toLowerCase()));
  function openHistoryDetail(event) {
    const item = resolveCardDetailPrinting(event, trackedItems);
    if (item) setDetail(item);
  }
  const columns = [
    { key: "name", label: "Card", minWidth: 170, widthGrow: 1, widthShrink: 1, render: event => resolveCardDetailPrinting(event, trackedItems) ? <button className="table-link" aria-label={`Open details for ${event.name}, ${printing(event)}`} onClick={() => openHistoryDetail(event)}><CardIdentity item={event} /></button> : <CardIdentity item={event} /> },
    { key: "set_code", label: "Set", align: "center", width: 48, widthShrink: 0, format: item => String(item.set_code || "-").toUpperCase() },
    { key: "collector_number", label: "#", align: "center", width: 48, widthShrink: 0, format: item => item.collector_number || "-" },
    { key: "rarity", label: "Rarity", align: "center", width: 44, widthShrink: 0, format: rarity },
    { key: "color", label: "Color", align: "center", width: 44, widthShrink: 0, format: color },
  ...
  return <><ViewHeader title="History" description="Recent activity and learning trail across transactions, radar, and notes." /><MetricBand items={[{ label: "Events", value: allEvents.length, detail: "Reviewable records" }, { label: "Trades", value: state.transactions.length, detail: "Buy and sell events" }, { label: "Lessons / Review", value: state.cardNotes.length + (state.thesisNotes || []).length, detail: "Saved decision notes" }, { label: "Notes", value: state.cardNotes.length, detail: "Card memory" }]} /><TableFilterPanel title="Filter History" countText={`${events.length} ${events.length === 1 ? "event" : "events"}`} value={filter} onChange={value => { if (focusId) setParams({}); setFilter(value); }} placeholder="Event, card, detail" pageSize={pageSize} onPageSizeChange={setPageSize} onReset={() => { setParams({}); setFilter(""); setType(""); }}><label className="filter-control"><span>Type</span><select aria-label="History type" value={type} onChange={event => setType(event.target.value)}><option value="">All events</option><option value="transaction">Transactions</option><option value="radar">Radar</option><option value="note">Notes</option></select></label>{focusId && <button type="button" className="filter-reset-btn" onClick={() => setParams({})}>Show all History</button>}</TableFilterPanel><TabulatorTable columns={columns} rows={events} onRowClick={openHistoryDetail} tableClass="ms-tabulator--history" ariaLabel="Activity history" empty="Activity will appear here as you work." initialSort={[{ column: "name", dir: "asc" }]} pageSize={pageSize} /><CardDetail item={detail} source="positions" onClose={() => setDetail(null)} /></>;
}
