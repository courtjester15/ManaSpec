...
export function TransactionsView() {
  const { state } = useAppState(); const [params, setParams] = useSearchParams(); const [filter, setFilter] = useState(""); const [type, setType] = useState(""); const [pageSize, setPageSize] = useState(25); const [detail, setDetail] = useState(null); const focusId = params.get("focus") || ""; const rows = state.transactions.filter(tx => (!focusId || String(tx.id) === focusId) && (!type || tx.type === type) && [tx.name, tx.set_code, tx.notes, tx.type].join(" ").toLowerCase().includes(filter.toLowerCase()));
  ...
  return <><ViewHeader title="Transactions" description="Compact ledger for buys, sells, balances, fees, and realized outcomes." /><MetricBand items={[{ label: "Buys", value: buys.length, detail: `${formatMoney(bought)} deployed` }, { label: "Sells", value: sells.length, detail: `${formatMoney(sold)} returned` }, { label: "Net Cash Flow", value: signedMoney(sold - bought), detail: "Sells minus buys" }, { label: "Recent Activity", value: state.transactions[0] ? date(state.transactions[0].date) : "-", detail: state.transactions[0]?.name || "No transactions" }]} /><TableFilterPanel title="Filter Transactions" countText={`${rows.length} ${rows.length === 1 ? "transaction" : "transactions"}`} value={filter} onChange={value => { if (focusId) setParams({}); setFilter(value); }} placeholder="Card, set, notes" pageSize={pageSize} onPageSizeChange={setPageSize} onReset={() => { setParams({}); setFilter(""); setType(""); }}><label className="filter-control"><span>Type</span><select aria-label="Transaction type" value={type} onChange={event => setType(event.target.value)}><option value="">All types</option><option>BUY</option><option>SELL</option></select></label>{focusId && <button type="button" className="filter-reset-btn" onClick={() => setParams({})}>Show all Transactions</button>}</TableFilterPanel><TabulatorTable columns={columns} rows={rows} onRowClick={item => { const printingItem = resolveCardDetailPrinting(item, [...state.specs, ...state.radar]); if (printingItem) setDetail({ ...printingItem, currentPrice: printingItem.currentPrice ?? item.price }); }} tableClass="ms-tabulator--transactions" ariaLabel="Transactions ledger" empty="No transactions yet. Buys and sells will appear here automatically." initialSort={[{ column: "name", dir: "asc" }]} pageSize={pageSize} /><CardDetail item={detail} source="positions" onClose={() => setDetail(null)} /></>;
}
