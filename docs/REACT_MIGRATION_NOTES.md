...
- React Parity Batches 1 and 2 close the audit's C1/C2 data-trust findings and H1/H2 Signals workflow findings with focused regression coverage. Signals now uses vanilla's shared 5% derivation, exact market-check identity, Dashboard queue membership, triage filters, and exact source navigation; Card Detail workflow and dense-table review controls remain open for later batches.
