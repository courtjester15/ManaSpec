...
- The shared Tabulator foundation is implemented and vanilla-verified with Radar as the Phase 1 pilot. It centralizes lifecycle, compact geometry, pagination, sort accessibility, indicators, and actions while routes retain their filters, columns, editors, and workflows. Positions is the next deliberate Phase 2 migration; other table modules remain unchanged until their own focused migrations.
...
- The next phase applies the approved foundation to Positions without changing business logic, then closes remaining workflow/UI/accessibility/responsive gaps and evaluates the next useful libraries against the working baseline.
