...
- The shared Tabulator foundation is implemented and vanilla-verified with Radar as the Phase 1 pilot and Positions as the first focused Phase 2 consumer. It centralizes lifecycle, compact geometry, pagination, sort accessibility, indicators, and actions while routes retain their filters, columns, editors, and workflows. Signals, Transactions, and History remain on the interim table until separately approved migrations.
