...
A single primary table direction will serve Radar, Positions, Signals, Transactions, and History. Radar uses the adopted and vanilla-verified wrapper now. The interim native `DataTable` remains in the other modules only until their deliberate Phase 2 migrations, not as a competing long-term system. A second long-term table system requires a documented exception.
