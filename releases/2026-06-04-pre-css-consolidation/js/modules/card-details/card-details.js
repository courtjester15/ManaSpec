...
function renderCardDetail(item, card, source) {
  ...
  document.getElementById("cardDetailBody").innerHTML = `
    ...
    ${renderActionsSection(marketLinks)}
    ${renderMarketEvaluationSection(item, latestTcg, Boolean(owned))}
    ${renderMarketCheckSection(item, latestTcg)}
...
function renderCardDetailHeader(item) {
  return `
    <header class="detail-header">
      <div>
        <h3>${item.name}</h3>
        <span>${item.set_code} #${String(item.collector_number || "").padStart(3, "0")} - ${item.set_name}</span>
      </div>
...
function renderPlanSection(item, targetState) {
  return `
    <section class="detail-command-section">
      <div class="detail-section-heading">
        <h4>Plan</h4>
        <span>${targetState.label}</span>
      </div>
      <div class="detail-grid plan-grid">
      ${renderDetailMetric("Entry", item.entryTarget ? money(item.entryTarget) : "-")}
      ${renderDetailMetric("Exit", item.exitTarget ? money(item.exitTarget) : "-")}
      ${renderDetailMetric("Target", targetState.label)}
      ${renderDetailMetric("Hold", item.holdTime || "-")}
      </div>
      <form class="target-plan-form" id="targetPlanForm">
        <label>
          Entry $
          <input id="entryTargetInput" type="text" inputmode="numeric" pattern="[0-9]*" value="${formatTargetInputNumber(item.entryTarget)}" placeholder="$">
        </label>
        <label>
          Exit $
          <input id="exitTargetInput" type="text" inputmode="numeric" pattern="[0-9]*" value="${formatTargetInputNumber(item.exitTarget)}" placeholder="$">
        </label>
        <label>
          Hold mo
          <input id="holdTimeInput" type="text" inputmode="numeric" pattern="[0-9]*" value="${formatHoldInputNumber(item.holdTime)}" placeholder="6">
        </label>
...
function renderThesisSection(item) {
  ...
  return `
    <section class="detail-command-section">
      <div class="detail-section-heading">
        <h4>Thesis</h4>
        <span>${notes.length ? `${notes.length} linked notes` : "No linked notes"}</span>
      </div>
      ...
      <div class="card-thesis-list">
        ${notes.length ? notes.slice(0, 3).map(renderLinkedThesisNote).join("") : ""}
      </div>
    </section>
  `;
}
...
function renderMarketCheckSection(item, latestTcg) {
  return `
    <section class="detail-command-section signal-section">
      <div class="detail-section-heading">
        <h4>Market Check</h4>
        <span>${latestTcg ? `Last saved ${new Date(latestTcg.checkedAt).toLocaleString()}` : "No TCG check saved yet"}</span>
      </div>
      <div class="signal-grid">
        ${renderSignalSlot("TCG Qty", formatSellerQty(latestTcg))}
        ${renderSignalSlot("TCG Price", formatMarketRecent(latestTcg))}
        ${renderSignalSlot("CK Buylist", "Needs Card Kingdom data source")}
        ${renderSignalSlot("Checked", item.priceUpdatedAt ? new Date(item.priceUpdatedAt).toLocaleDateString() : "-")}
      </div>
      <div class="market-paste-panel">
        <div class="market-check-note">
          Open the exact TCGplayer page, copy the visible Price Points block, then paste it here to save seller count, quantity, market price, recent sale, and velocity notes.
        </div>
        <textarea id="tcgPricePaste" placeholder="Paste the TCGplayer Price Points block here..."></textarea>
        <div class="market-paste-actions">
          <button type="button" id="saveTcgObservation">Save TCG Check</button>
          <span id="tcgParseStatus">${latestTcg ? `Last saved ${new Date(latestTcg.checkedAt).toLocaleString()}` : "No TCG check saved yet"}</span>
        </div>
        <div class="market-observation-preview" id="tcgObservationPreview">
          ${renderMarketObservation(latestTcg)}
        </div>
      </div>
    </section>
  `;
...
function renderMarketEvaluationSection(item, observation, owned) {
  ...
  return `
    <section class="detail-command-section">
      ...
      ${evaluation.gaps.length ? `<div class="evaluation-gaps">${evaluation.gaps.join(" / ")}</div>` : ""}
    </section>
  `;
}
...
function renderCardDataSection(item, card) {
  return `
    <section class="detail-command-section">
      <div class="detail-section-heading">
        <h4>Card Data</h4>
        <span>Printing identity and Oracle text</span>
      </div>
      <div class="detail-grid card-data-grid">
        ${renderDetailMetric("Rarity", item.rarity || "-")}
        ${renderDetailMetric("Color", getColorLabel(item))}
        ${renderDetailMetric("Type", getPrimaryType(item))}
        ${renderDetailMetric("Release", item.released_at || "-")}
        ${renderDetailMetric("Reserved", item.reserved ? "Yes" : "No")}
        ${renderDetailMetric("Reprint", item.reprint ? "Yes" : "No")}
      </div>
...
function getMarketLinks(card, item) {
  ...
  return [
    { label: "Open in TCGplayer", href: tcgLink },
    { label: "Open in CK Store", href: cardKingdomSearch },
    { label: "Open in CK Buylist", href: cardKingdomBuylist },
    { label: "Open in Scryfall", href: card.scryfall_uri || `https://scryfall.com/search?q=${encodeURIComponent(item.name)}` },
  ];
}
...
function buildMarketEvaluation(item, observation, owned) {
  ...
  const gaps = getMarketEvaluationGaps(item, observation);

  return {
    summary: [supply.value, velocity.value, price.value].join(" / "),
    gaps,
    items: [
      supply,
      velocity,
...
function evaluateSupply(observation) {
  if (!observation) {
    return buildEvaluationItem("Supply", "No check", "Paste TCG Price Points", "muted");
  }
  ...
  return buildEvaluationItem("Supply", value, `${sellers || 0}s / ${qty || 0}q / ${perSeller ? perSeller.toFixed(1) : "0"} ea`, tone);
}

function evaluateVelocity(observation) {
  if (!observation) {
    return buildEvaluationItem("Velocity", "No check", "No sales velocity pasted", "muted");
  }

  const daily = Number(observation.avgDailySold || 0);
  const total = Number(observation.totalSold || 0);

  let value = "No visible velocity";
  let tone = "risk";
  if (daily >= 2) {
    value = "Active";
    tone = "good";
  ...
  return buildEvaluationItem("Velocity", value, `${daily || 0}/day / ${total || 0} sold`, tone);
}

function evaluatePriceConfidence(observation) {
  if (!observation) {
    return buildEvaluationItem("Price", "No check", "No market/recent/median data", "muted");
  }
  ...
  return buildEvaluationItem("Price", value, `M ${formatOptionalMoney(market || null)} / R ${formatOptionalMoney(recent || null)} / Med ${formatOptionalMoney(median || null)}`, tone);
}
...
function evaluateEdhPresence(item) {
  ...
  if (!rank) {
    return buildEvaluationItem("EDH", "N/A", "No Scryfall rank", "muted");
  }
...
function evaluateTargetMath(item, owned) {
  ...
  if (!price || (!entry && !exit)) {
    return buildEvaluationItem("Target", "No target", "Entry/exit targets are optional", "muted");
  }
...
function evaluateFreshness(observation) {
  if (!observation?.checkedAt) {
    return buildEvaluationItem("Freshness", "No check", "No saved market observation", "muted");
  }
...
function getMarketEvaluationGaps(item, observation) {
  const gaps = [];
  if (!observation) gaps.push("No TCG check");
  if (!Number(item.entryTarget || 0) && !Number(item.exitTarget || 0)) gaps.push("No target");
  if (typeof getThesisNotesForCard === "function" && !getThesisNotesForCard(item.id).length) gaps.push("No linked thesis");
  if (observation && !Number(observation.avgDailySold || 0)) gaps.push("No velocity");
  if (observation && !Number(observation.mostRecentSale || 0)) gaps.push("No recent sale");
  return gaps;
}
